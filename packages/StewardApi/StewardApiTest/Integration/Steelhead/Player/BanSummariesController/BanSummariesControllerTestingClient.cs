...
namespace Turn10.LiveOps.StewardTest.Integration.Steelhead
{
    public sealed class BanSummariesControllerTestingClient : SteelheadStewardBaseTestingClient
    {
        ...
        {
            var path = new Uri(this.baseUri, $"{TitlePath}/player/banSummaries");

            return await ServiceClient.SendRequestAsync<IList<BanSummary>>(HttpMethod.Post, path, this.authKey, Version, headers: this.headers).ConfigureAwait(false);
        }
    }
}
