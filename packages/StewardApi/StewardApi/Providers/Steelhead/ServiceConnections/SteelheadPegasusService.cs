...
namespace Turn10.LiveOps.StewardApi.Providers.Steelhead.ServiceConnections
{
    ...
    public sealed class SteelheadPegasusService : ISteelheadPegasusService
    {
        ...
        {
            environment = this.CoalesceNullEmptyOrWhitespace(environment, this.defaultCmsEnvironment);
            slot = this.CoalesceNullEmptyOrWhitespace(slot, this.defaultCmsSlot);
            snapshot = this.DefaultNullFromEmptyOrWhitespace(snapshot);
                ...
                foreach (var supportedLocale in supportedLocales)
                {
                    ...
                    foreach (var locStringKey in localizedStrings.Keys)
                    {
                        ...
                        var localizedResult = new LiveOpsContracts.LocalizedString()
                        {
                            ...
                            SubCategory = localizedStrings[locStringKey].SubCategory,
        ...
        {
            var allPrs = await this.azureDevOpsManager.GetPullRequestsIntoPlaytestBranchAsync(status, null).ConfigureAwait(false);
        ...
        private string CoalesceNullEmptyOrWhitespace(string input, string defaultValue)
        {
            return string.IsNullOrWhiteSpace(input) ? defaultValue : input;
        }

        private string DefaultNullFromEmptyOrWhitespace(string input)
        {
