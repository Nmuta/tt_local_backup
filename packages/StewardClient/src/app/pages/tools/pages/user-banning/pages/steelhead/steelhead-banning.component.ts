...
import { SteelheadBanHistoryComponent } from '@shared/views/ban-history/steelhead/steelhead-ban-history.component';
...
import { SteelheadPlayersBanService } from '@services/api-v2/steelhead/players/ban/steelhead-players-ban.service';
...
export class SteelheadBanningComponent extends UserBanningBaseComponent implements OnInit {
  @ViewChildren('steelhead-ban-history')
  public banHistoryComponents: SteelheadBanHistoryComponent[] = [];
  ...
  constructor(
    ...
    private readonly steelheadPlayersBanService: SteelheadPlayersBanService,
    private readonly steelheadPlayerBanService: SteelheadPlayerBanService,
  ) {
      ...
      return summaryLookup$.pipe(
        switchMap(summaryLookup => {
          return of(
            chain(identities)
              .sortBy(i => {
                const banCount = summaryLookup[i?.steelhead?.xuid?.toString()]?.banCount;
                return banCount;
              })
              .reverse()
              .value(),
  ...
  public ngOnInit(): void {
    const getBanConfigurations$ = this.steelheadPlayersBanService.getBanConfigurations$();

    const getBanReasonGroups$ = this.steelheadPlayersBanService.getBanReasonGroups$();
  ...
  public submitBan(): void {
    ...
    this.steelheadPlayersBanService
  ...
  private updateNextBanDuration() {
    ...
    this.nextBanDurationSubscription = this.steelheadPlayerBanService
