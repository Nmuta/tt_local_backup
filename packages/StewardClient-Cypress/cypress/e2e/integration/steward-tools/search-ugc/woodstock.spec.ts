...
import { clickSearch, selectCar, selectCuratedUgcType, selectUgcType, orderUgcBy } from './shared-tests';
...
context('Steward / Tools / UGC Details / Woodstock', () => {
  ...
  it('Should be able to hide, report, or generate share codes for ugc', () => {
    ...
    cy.contains('button', 'Hide').parent().within(() =>{
    ...
    cy.contains('button', 'Report').parent().within(() =>{
      cy.get('verify-button').click();
    })
    cy.contains('button', 'Report').should('be.enabled');

    // Generate share code for Ugc
    cy.contains('button', 'Generate').parent().within(() =>{
      cy.get('verify-button').click();
    })
