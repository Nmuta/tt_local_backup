...
export class WoodstockPlayerXuidAuctionsFakeApi extends FakeApiBase {
  ...
  public get canHandle(): boolean {
    ...
    const regex = /^\/?api\/v1\/title\/woodstock\/player\/xuid\((\d+)\)\/auctions$/i;
    return regex.test(url.pathname);
  }

  /** Produces a sample API response. */
  public handle(): PlayerAuction[] {
    return WoodstockPlayerXuidAuctionsFakeApi.makeMany();
  }
