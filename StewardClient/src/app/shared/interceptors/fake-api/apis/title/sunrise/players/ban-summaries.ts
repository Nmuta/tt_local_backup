    ...
    return xuids.map(xuid => {
      return <SunriseBanSummary>{
        ...
        lastBanDescription: {
          ...
          lastExtendedReason: faker.random.words(faker.random.number({ min: BigInt(5), max: BigInt(50) })),
          lastExtendedTimeUtc: faker.date.past(),
          reason: faker.random.words(faker.random.number({ min: BigInt(5), max: BigInt(50) })),
