...
export class SteelheadPlayerXuidAuctionsFakeApi extends FakeApiBase {
  ...
  public get canHandle(): boolean {
    ...
    const regex = /^\/?api\/v1\/title\/steelhead\/player\/xuid\((\d+)\)\/auctions$/i;
    return regex.test(url.pathname);
  }

  /** Produces a sample API response. */
  public handle(): PlayerAuction[] {
    return SteelheadPlayerXuidAuctionsFakeApi.makeMany();
  }
