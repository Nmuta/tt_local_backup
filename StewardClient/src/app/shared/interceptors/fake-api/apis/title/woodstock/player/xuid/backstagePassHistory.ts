...
export class WoodstockPlayerXuidBackstagePassHistoryFakeApi extends FakeApiBase {
  ...
  public get canHandle(): boolean {
    ...
    const regex = /^\/?api\/v1\/title\/woodstock\/player\/xuid\((\d+)\)\/backstagePassUpdates$/i;
    return regex.test(url.pathname);
  }

  /** Produces a sample API response. */
  public handle(): BackstagePassHistory[] {
    return WoodstockPlayerXuidBackstagePassHistoryFakeApi.makeMany();
  }
