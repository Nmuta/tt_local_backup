...
@Component({
  selector: 'woodstock-lsp-group-selection',
  templateUrl: '../lsp-group-selection.component.html',
  styleUrls: ['../lsp-group-selection.component.scss'],
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => WoodstockLspGroupSelectionComponent),
      multi: true,
    },
  ],
})
export class WoodstockLspGroupSelectionComponent extends LspGroupSelectionBaseComponent {
  ...
  /** Creates Woodstock's player selection request. */
  public dispatchLspGroupStoreAction(): void {
    this.store.dispatch(new GetLspGroups(GameTitleCodeName.FH5));
  }

  /** Lsp group state property selector */
  public lspGroupSelector$(): Observable<LspGroups> {
    return this.store.select(LspGroupMemoryState.WoodstockLspGroups);
  }
}
