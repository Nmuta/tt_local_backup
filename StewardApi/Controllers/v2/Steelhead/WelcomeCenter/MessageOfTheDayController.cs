...
namespace Turn10.LiveOps.StewardApi.Controllers.V2.Steelhead.WelcomeCenter
{
    ...
    public class MessageOfTheDayController : V2SteelheadControllerBase
    {
        ...
        public async Task<IActionResult> EditAndSubmitMessageOfTheDay(string id, [FromBody] MessageOfTheDayBridge messageOfTheDayBridge)
        {
            if (!Guid.TryParse(id, out var parsedId))
            {
                throw new BadRequestStewardException($"ID could not be parsed as GUID. (id: {id})");
            ...
            GitPullRequest pr = await this.steelheadPegasusService.CreatePullRequestAsync(pushed, pullRequestTitle, pullRequestDescription).ConfigureAwait(true);
