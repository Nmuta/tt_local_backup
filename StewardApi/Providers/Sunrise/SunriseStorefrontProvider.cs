...
namespace Turn10.LiveOps.StewardApi.Providers.Sunrise
{
    ...
    public sealed class SunriseStorefrontProvider : ISunriseStorefrontProvider
    {
        ...
        /// <inheritdoc />
        public async Task<IList<UGCItem>> SearchUGCItems(UGCType ugcType, UGCFilters filters)
        {
            ...
            {
                case UGCType.Livery:
                    var nonMappedLiveries = await this.sunriseService.GetPlayerLiveries(mappedFilters).ConfigureAwait(false);
                    ugcItems = this.mapper.Map<List<UGCItem>>(nonMappedLiveries.result);
                    break;
                case UGCType.Photo:
                    var nonMappedPhotos = await this.sunriseService.GetPlayerPhotos(mappedFilters).ConfigureAwait(false);
        ...
        /// <inheritdoc />
        public async Task<UGCItem> GetUGCLivery(Guid liveryId)
        {
            var liveryOutput = await this.sunriseService.GetPlayerLivery(liveryId).ConfigureAwait(false);

            return this.mapper.Map<UGCItem>(liveryOutput.result);
        }

        /// <inheritdoc />
        public async Task<UGCItem> GetUGCPhoto(Guid photoId)
        {
            var photoOutput = await this.sunriseService.GetPlayerPhoto(photoId).ConfigureAwait(false);

            return this.mapper.Map<UGCItem>(photoOutput.result);
        }

        /// <inheritdoc />
        public async Task SetUGCFeaturedStatus(Guid contentId, bool isFeatured, TimeSpan? featuredExpiry)
        {
            try
            {
                var featureEndDate = isFeatured && featuredExpiry.HasValue ? DateTime.UtcNow.Add(featuredExpiry.Value) : DateTime.MinValue;
                await this.sunriseService.SetUGCFeaturedStatus(contentId, isFeatured, featureEndDate).ConfigureAwait(false);
            }
            catch (Exception ex)
            {
