...
namespace Turn10.LiveOps.StewardApi.Providers.Steelhead
{
    ...
    public sealed class SteelheadGiftHistoryProvider : ISteelheadGiftHistoryProvider
    {
        ...
        /// <inheritdoc />
        public async Task UpdateGiftHistoryAsync(string id, string title, string requesterObjectId, GiftIdentityAntecedent giftHistoryAntecedent, SteelheadGift gift)
        {
        ...
        private async Task<IList<SteelheadGiftHistory>> GetGiftHistoriesAsync(string playerId, string title)
        {
            ...
            foreach (var history in giftHistoryResult)
            {
                ...
                results.Add(new SteelheadGiftHistory(antecedent, id, history.Title, history.RequesterObjectId, history.GiftSendDateUtc, convertedGift));
            }
